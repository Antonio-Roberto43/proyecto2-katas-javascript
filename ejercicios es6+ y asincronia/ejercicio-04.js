...
const stramersU = streamers.filter(streamer => streamer.name.includes('u'));

console.log(stramersU);

/* 4.5 utiliza .filter() para generar un nuevo array con los streamers que incluyan el caracter 'Legends' en su propiedad .gameMorePlayed. Recomendamos usar la funcion
...
const streamersUpper = streamers.filter(streamer => streamer.gameMorePlayed.includes('Legends')).map(streamer => {
    if (streamer.age > 35) {
        return {
            ...streamer, 
